Feature: Account registry
    ...
    Scenario: User is able to update name of already created account
        Given Account registry is empty
        And I create an account using name: "john", last name: "doe", pesel: "88050512345"
        When I update "name" of account with pesel: "88050512345" to "jane"
        Then Account with pesel "88050512345" has "name" equal to "jane"

    Scenario: Created account has all fields correctly set
        Given Account registry is empty
        When I create an account using name: "anna", last name: "kowalska", pesel: "92031567890"
        Then Account with pesel "92031567890" exists in registry
        And Account with pesel "92031567890" has "name" equal to "anna"
